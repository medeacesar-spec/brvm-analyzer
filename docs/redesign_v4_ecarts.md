...
| # | Onglet | Écart | État |
|---|---|---|---|
| A1 | Cours | Pas de sélecteur **Exercice** dans la barre d'outils | **arbitré le 09/09 : sur Fondamentale, pas sur Cours.** Fait — le sélecteur ne gouverne que cet onglet, et quand l'exercice choisi diffère de celui de la fiche, l'onglet le **dit** : le score, le verdict et la recommandation de la page restent sur l'exercice de la fiche |
...
| A6 | Technique | Pas de sélecteur **Périodicité** (Journalière / Mensuelle) | **arbitré le 09/09 : ne pas le faire.** `price_monthly` n'a ni ouverture, ni plus haut, ni plus bas — les chandeliers deviendraient une courbe et les indicateurs changeraient de sens. L'application continue de **détecter** la périodicité |
...
| F2 | Par secteur | « Performance sectorielle » rendue en **graphique de la moyenne** ; le canevas met un **tableau à six colonnes** : Titres, Moyenne, **Médiane**, Position (barre signée), **Étendue**. La médiane et l'étendue disent si la moyenne est représentative ou tirée par un titre — l'application tient déjà ce raisonnement dans l'onglet Risque | **fait** — vu au rendu le 09/09. Le graphique de la moyenne ET le tableau brut qui la répétait laissent place à une seule table à six colonnes. Elle a révélé une ligne **sans nom de secteur** : SVOC.ci, retiré de la cote, figurait encore au classement |
| F3 | — | Quatre blocs hors canevas : le **curseur** du nombre de titres, le **sélecteur de secteur** en descente, « Rendement rapporté au risque · 5 ans » et « Comparaison secteurs ». Le sélecteur de période offre par ailleurs 3M/6M/1A/2A/3A/Max là où le canevas propose 1 mois/3 mois/1 an/3 ans/5 ans | **arbitré le 09/09 : les garder.** Le curseur, le sélecteur de secteur, « Rendement rapporté au risque » et « Comparaison secteurs » restent. Le troisième dit ce qu'aucune performance de période ne dit — ce qu'un gain a coûté en agitation. Écart assumé, comme les cartes de contradiction des Signaux |
...
| H3 | Deux boutons hors canevas sur « Données brutes » : **Exporter JSON**, que le canevas n'a pas, et **Colonnes**, désactivé avec l'infobulle « à venir » — un contrôle qui annonce une fonction inexistante | **arbitré le 09/09 : les retirer.** Fait. « Colonnes » était désactivé avec l'infobulle « à venir » — un contrôle qui annonce une fonction inexistante. « Exporter JSON » fonctionnait ; il part avec, le CSV reste |
